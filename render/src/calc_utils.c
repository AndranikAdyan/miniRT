...
/*   Updated: 2025/08/07 21:46:35 by saslanya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec	sphere_normalize(t_sphere *sphere, const t_vec *hit_point)
{
	...
	pms[2] = get_bump_height(&sphere->bump, pms[0], pms[1]);
	...
	vecs[2] = vec_add(p, vec_add(scalar_product(vecs[0], (get_bump_height
						(&sphere->bump, pms[0] + 0.005, pms[1])
						- pms[2]) * 0.2), scalar_product(vecs[1],
					(get_bump_height(&sphere->bump, pms[0], pms[1] + 0.005)
						- pms[2]) * 0.2)));
